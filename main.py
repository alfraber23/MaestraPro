...
class SistemaEscolarApp:
    ...
    def mostrar_login(self):
        ...
        tk.Button(frame, text="Entrar", bg=COLOR_SUCCESS, fg="white",
                  command=self.accion_login).pack(fill="x", pady=15)
        tk.Button(frame, text="Registrarse", fg="blue",
                  relief="flat", command=self.mostrar_registro).pack()
    ...
    def mostrar_registro(self):
        ...
        tk.Button(frame, text="Registrar", bg=COLOR_PRIMARY, fg="white",
                  command=self.accion_registro).pack(fill="x", pady=15)
    ...
    def mostrar_dashboard(self):
        ...
        header.pack(fill="x")
        tk.Label(header, text="Mis Ciclos Escolares", fg="white",
                 bg=COLOR_BG_DARK, font=FONT_TITLE).pack(side="left", padx=15, pady=10)
        tk.Button(header, text="Salir", bg="#F44336", fg="white",
                  command=self.mostrar_login).pack(side="right", padx=15)

        # Body
        body = tk.Frame(self.root, padx=20, pady=20)
        body.pack(fill="both", expand=True)

        tk.Button(body, text="+ Nuevo Grupo", bg=COLOR_PRIMARY, fg="white",
        ...
        for c in cols:
            self.tree_ciclos.heading(c, text=c)
    ...
    def cargar_ciclos(self):
        for i in self.tree_ciclos.get_children():
            self.tree_ciclos.delete(i)
        lista = obtener_ciclos_por_usuario(self.usuario_id)
        for c in lista:
            # c = (id, nombre, tareas, trabajos, proyecto, valores, examen)
            self.tree_ciclos.insert("", "end", values=(
                c[0], c[1], f"{c[2]}%", f"{c[3]}%", f"{c[6]}%"))
    ...
    def popup_crear_ciclo(self):
        ...
        VentanaNuevoCiclo(self.root, self.usuario_id,
                          self.callback_ciclo_creado)
    ...
    def seleccionar_ciclo(self, event):
        ...
        if self.ciclo_actual_encuadre is None:
            self.ciclo_actual_encuadre = {
                'tareas': 10, 'trabajos': 45, 'proyecto': 5, 'valores': 20, 'examen': 20}
    ...
    def mostrar_vista_grupo(self):
        ...
        header.pack(fill="x")
        tk.Button(header, text="< Volver", bg="#1976D2", fg="white", relief="flat",
                  command=self.mostrar_dashboard).pack(side="left", padx=10, pady=10)
        tk.Label(header, text=f"Grupo: {self.ciclo_actual_nombre}", fg="white",
                 bg=COLOR_PRIMARY, font=FONT_TITLE).pack(side="left", padx=5)
        ...
        tk.Button(acciones, text="+ Alumno", bg=COLOR_SUCCESS, fg="white",
                  command=self.popup_add_alumno).pack(side="left", padx=5)

        # BOTON PARA TOMAR ASISTENCIA
        tk.Button(acciones, text="Pasar Lista", bg="#009688", fg="white",
                  command=self.abrir_ventana_asistencia).pack(side="left", padx=5)

        # NUEVO BOTÓN PARA VER BOLETA
        tk.Button(acciones, text="Ver Boleta", bg="#9C27B0", fg="white",
                  command=self.abrir_ventana_boleta).pack(side="left", padx=5)

        # --- BOTÓN NARANJA CONECTADO ---
        tk.Button(acciones, text="Capturar Calificaciones", bg="#FF9800", fg="black",
                  command=self.abrir_ventana_calificar).pack(side="left", padx=5)

        tk.Label(body, text="Selecciona un alumno para calificar:",
        ...
        self.tree_alumnos.heading("Nombre", text="Nombre Alumno")
        self.tree_alumnos.column("Nombre", width=400)
        self.tree_alumnos.pack(fill="both", expand=True)

        self.cargar_alumnos()

    def abrir_ventana_asistencia(self):
        VentanaAsistencia(self.root, self.ciclo_actual_id,
                          self.ciclo_actual_nombre)
    ...
    def abrir_ventana_calificar(self):
        ...
        if self.ciclo_actual_encuadre is None:
            messagebox.showerror(
                "Error Crítico", "No se cargó la configuración del ciclo.\nIntenta volver al menú principal y entrar de nuevo.")
            return
        # 1. Validar selección
        seleccion = self.tree_alumnos.selection()
        if not seleccion:
            messagebox.showwarning(
                "Atención", "Primero selecciona un alumno de la lista.")
            return

        datos_alumno = self.tree_alumnos.item(
            seleccion, "values")  # (ID, Nombre)
        id_alumno = int(datos_alumno[0])
        nombre_alumno = datos_alumno[1]

        # 2. Abrir Ventana Popup (Toplevel)
        VentanaCalificar(self.root, id_alumno, nombre_alumno,
                         self.ciclo_actual_id, self.ciclo_actual_encuadre)

    def abrir_ventana_boleta(self):
        seleccion = self.tree_alumnos.selection()
        if not seleccion:
            messagebox.showwarning(
                "Atención", "Selecciona un alumno para ver su boleta.")
            return
        ...
        VentanaBoleta(self.root, id_alumno, nombre_alumno,
                      self.ciclo_actual_id, self.ciclo_actual_encuadre)
...
class VentanaAsistencia:
    def __init__(self, parent, ciclo_id, nombre_ciclo):
        ...
        tk.Button(frame_top, text="Cargar Historial",
                  command=self.cargar_lista).pack(side="left", padx=10)
        tk.Button(self.window, text="GUARDAR ASISTENCIA", bg="#009688", fg="white", font=(
            "Arial", 12, "bold"), command=self.guardar_cambios).pack(fill="x", padx=20, pady=20)

        # --- Botones de Selección Rápida ---
        frame_tools = tk.Frame(self.window)
        frame_tools.pack(fill="x", padx=20)
        tk.Button(frame_tools, text="Marcar Todos",
                  command=self.marcar_todos, font=("Arial", 8)).pack(side="left")
        tk.Button(frame_tools, text="Desmarcar Todos", command=self.desmarcar_todos, font=(
            "Arial", 8)).pack(side="left", padx=5)
    ...
    def cargar_lista(self):
        ...
        historial = obtener_asistencia_fecha(
            self.ciclo_id, fecha)  # {id: 1, id: 0}
        ...
        for al_id, al_nombre in alumnos:
            ...
            c = tk.Checkbutton(self.scrollable_frame, text=al_nombre,
                               variable=var, font=("Arial", 10), anchor="w")
    ...
    def marcar_todos(self):
        for var in self.vars_asistencia.values():
            var.set(1)
...
class VentanaNuevoCiclo:
    def __init__(self, parent, usuario_id, callback_exito):
        ...
        # --- UI ---
        tk.Label(self.window, text="Configuración del Grupo", font=(
            "Arial", 14, "bold"), fg="#2196F3").pack(pady=15)

        # Nombre
        tk.Label(self.window, text="Nombre del Grupo (Ej. Matemáticas A):").pack(
            anchor="w", padx=20)
        self.ent_nombre = tk.Entry(self.window)
        self.ent_nombre.pack(fill="x", padx=20, pady=5)

        tk.Label(self.window, text="Defina los porcentajes de evaluación (Suma = 100%):", font=(
            "Arial", 9, "bold")).pack(pady=(15, 5))
        ...
        for i, (key, label) in enumerate(criterios):
            ...
            tk.Label(row, text=f"{label}:", width=20,
    ...
    def guardar_ciclo(self):
        ...
        if not nombre:
            messagebox.showwarning(
                "Error", "Debes ponerle un nombre al grupo.")
            return
        ...
        try:
            ...
            if abs(total - 100.0) > 0.1:  # Usamos tolerancia pequeña por decimales flotantes
                messagebox.showerror(
                    "Error Matemático", f"Los porcentajes suman {total}%. \nDeben sumar exactamente 100%.")
                return
            ...
            messagebox.showinfo(
                "Éxito", f"Grupo '{nombre}' creado correctamente.")
            self.callback()  # Actualizamos la tabla de atrás
            self.window.destroy()  # Cerramos esta ventana

        except ValueError:
            messagebox.showerror(
                "Error", "Asegúrate de ingresar solo números en los porcentajes.")
...
class VentanaBoleta:
    def __init__(self, parent, alumno_id, nombre, ciclo_id, encuadre):
        ...
        tk.Label(self.window, text=f"Boleta de Calificaciones",
                 font=("Arial", 14, "bold"), fg="#9C27B0").pack(pady=10)
        tk.Label(self.window, text=f"Alumno: {nombre}", font=(
            "Arial", 12)).pack()
        ...
        cols = ("Materia", "Periodo 1", "Periodo 2",
                "Periodo 3", "Promedio Final")
        ...
        # Botón Cerrar
        tk.Button(self.window, text="Cerrar",
    ...
    def cargar_datos(self, uid, cid, enc):
        ...
        if count > 0:
            ...
            self.tree.insert("", "end", values=(
                "PROMEDIO GRAL", "-", "-", "-", gral), tags=('total',))
            self.tree.tag_configure('total', font=(
                'Arial', 10, 'bold'), background='#E1BEE7')
...
class VentanaCalificar:
    def __init__(self, parent, alumno_id, nombre_alumno, ciclo_id, encuadre):
        ...
        # --- UI Elementos ---
        tk.Label(self.window, text=f"Alumno: {nombre_alumno}",
                 font=FONT_SUBTITLE, fg=COLOR_PRIMARY).pack(pady=10)
        ...
        self.combo_campo = ttk.Combobox(
            frame_sel, values=CAMPOS_FORMATIVOS, state="readonly")
        ...
        self.combo_periodo = ttk.Combobox(
            frame_sel, values=["1", "2", "3"], width=5, state="readonly")
        self.combo_periodo.current(0)
        self.combo_periodo.grid(row=0, column=3, padx=5)

        tk.Button(frame_sel, text="Cargar Datos", command=self.cargar_datos_existentes).grid(
        ...
        for crit in criterios:
            ...
            tk.Label(self.frame_form, text=f"{crit.capitalize()} ({pct}%):", font=(
                "Arial", 10, "bold")).grid(row=row, column=0, sticky="w", pady=5)
        ...
        tk.Frame(self.frame_form, height=2, bg="gray").grid(
            row=row, column=0, columnspan=2, sticky="ew", pady=10)
        row += 1

        pct_trab = self.encuadre.get("trabajos", 0)
        tk.Label(self.frame_form, text=f"TRABAJOS ({pct_trab}%):", font=(
            "Arial", 10, "bold"), fg=COLOR_PRIMARY).grid(row=row, column=0, sticky="w")
        row += 1

        # Input Configuración Global
        tk.Label(self.frame_form, text="Total Trabajos (Materia/Periodo):").grid(
            row=row, column=0, sticky="e")
        # Color diferente para notar que es config
        self.ent_max_trabajos = tk.Entry(self.frame_form, bg="#FFF3E0")
        self.ent_max_trabajos.grid(row=row, column=1, padx=10)
        row += 1

        # Input Alumno
        tk.Label(self.frame_form, text="Entregados por Alumno:").grid(
            row=row, column=0, sticky="e")
        ...
        self.lbl_resultado = tk.Label(
            self.window, text="Promedio: -", font=("Arial", 14, "bold"))
    ...
    def cargar_datos_existentes(self):
        ...
        # Aquí guardamos CANTIDAD entregada
        val_trab = notas.get("trabajos", 0.0)
    ...
    def guardar_datos(self):
        try:
            ...
            if max_t < 0:
                raise ValueError("Max trabajos no puede ser negativo")
            ...
            registrar_calificacion(self.alumno_id, campo,
                                   periodo, "trabajos", entregados)

            # Otros criterios
            for crit, ent in self.entries.items():
                val = float(ent.get())
                if val < 0 or val > 100:
                    raise ValueError(f"Valor incorrecto en {crit}")
                registrar_calificacion(
                    self.alumno_id, campo, periodo, crit, val)

            # 3. Calcular y Mostrar Promedio
            promedio = calcular_promedio_periodo(
                self.alumno_id, self.ciclo_id, campo, periodo, self.encuadre)
            self.lbl_resultado.config(
                text=f"Promedio P{periodo}: {promedio} (Guardado)")

            messagebox.showinfo(
                "Éxito", f"Calificación P{periodo}: {promedio}\nDatos guardados correctamente.")

        except ValueError as e:
            messagebox.showerror(
                "Error", f"Verifica los datos numéricos.\n{e}")


if __name__ == "__main__":
    # --- ESTA LÍNEA ES LA MAGIA ---
    # Antes de abrir la ventana, nos aseguramos de que la DB y las tablas existan
    inicializar_db()
